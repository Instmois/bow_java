...
public class GameServer {
    ...
    private static final double width = 363;
    ...
    public void sendWantToStart(PlayerHandler handler) {
        Action wantToStart = new Action(Action.Type.WantToStart, handler.getPlayerInfo().username);
    ...
    private void resetInfo() {
        ...
        gameInfo.bigTarget.y = 0.5 * height;
        gameInfo.bigTarget.direction = 1;
    ...
    private void next() {
        ...
        for (PlayerInfo p : gameInfo.playerList) {
            if (p.shooting) {
                ...
                if (hit(p.arrow, gameInfo.bigTarget)) {
                    ++p.score;
                    p.shooting = false;
                    p.arrow.x = 5.0;
                } else if (hit(p.arrow, gameInfo.smallTarget)) {
                    p.score += 2;
                    p.shooting = false;
                    p.arrow.x = 5.0;
